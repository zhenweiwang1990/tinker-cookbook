...
async def cli_main(cli_config: CLIConfig) -> None:
    ...
    # Initialize database
    from tinker_cookbook.recipes.cua_rl.database import init_database, get_session, get_session_direct
    ...
    async def do_cua_group_rollout_with_db(
        ...
        from tinker_cookbook.recipes.cua_rl.database_context import get_database_session, get_baseline_id, get_training_id
        from tinker_cookbook.recipes.cua_rl.database_training_hooks import (
            record_step_before_rollout,
            record_step_after_rollout,
        )
        from tinker_cookbook.recipes.cua_rl.database_dao import get_step_by_training_and_step, get_eval_by_training_and_step
        from tinker_cookbook.recipes.cua_rl.rollout import _rollout_step, _rollout_batch
    ...
    with get_session() as session:
        from tinker_cookbook.recipes.cua_rl.database_dao import get_latest_completed_step
    ...
    from tinker_cookbook.recipes.cua_rl.database_context import set_database_context
    ...
    async def run_evaluations_with_db(
        ...
        from tinker_cookbook.recipes.cua_rl.database_eval import (
            get_or_create_eval,
        ...
        try:
            ...
            if session and eval_id:
                try:
                    from tinker_cookbook.recipes.cua_rl.database_dao import update_eval
                    update_eval(
                        session,
                        eval_id,
                        status="failed",
        ...
        if save_every > 0 and i_batch > start_batch and i_batch % save_every == 0:
            try:
                ...
                if state_path:
                    ...
                    from tinker_cookbook.recipes.cua_rl.database_training_hooks import record_step_after_training
    ...
    try:
        ...
        from tinker_cookbook.recipes.cua_rl.database_context import clear_database_context
