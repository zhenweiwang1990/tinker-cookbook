from __future__ import annotations

from typing import List

from ... import config
from ....adb import AdbClient


class Task12Validator:
    def validate(self, adb_client: AdbClient) -> bool:
        queries: List[str] = [
            "SELECT 1 FROM (SELECT GROUP_CONCAT(listing_id, ',' ORDER BY created_at DESC) AS ids FROM messages ORDER BY created_at DESC LIMIT 5) t WHERE t.ids = '25016,25079,25058,25037,25019';"
        ]
        for sql in queries:
            output = adb_client.run_sqlite_query(
                package_name=config.get_package_name(),
                db_relative_path="databases/airbnbSQLiteSQLite.db",
                sql=sql,
            )
            print(output)
            if not output.strip():
                return False
        return True
